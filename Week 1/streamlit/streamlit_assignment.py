...
def render_employee():
    st.title("Employee Registration Page :male-technologist:")
        ...
        if empno and ename and job and deptno:
            st.session_state.employee_data.loc[
                len(st.session_state.employee_data)] = [empno, ename,
                                                        job, deptno]
            st.success("Employee has been added successfully!")
        else:
            st.error("All fields are required for adding an employee.")


def render_department():
    st.title("Department Registration Page")
...
def main():
    ...
    page_selection = st.sidebar.radio('Navigate to:',
                                      [
    ...
    if page_selection == "Department Data Entry":
        render_department()
    elif page_selection == "Employee Data Entry":
        render_employee()
